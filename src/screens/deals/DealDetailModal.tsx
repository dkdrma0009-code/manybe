...
const STATUS_OPTIONS = [
  ...
  { value: 'cancelled',   label: '취소됨',   bg: '#F3F4F6', color: '#9CA3AF' },
];

const STATUS_LABEL_TO_VALUE: Record<string, string> = {
  '검토중': 'pending', '협상중': 'in_progress', '계약완료': 'completed', '취소됨': 'cancelled',
};
...
export default function DealDetailModal({ visible, deal, onClose, onSuccess }: Props) {
  ...
  const [status, setStatus]   = useState(STATUS_LABEL_TO_VALUE[deal.status] ?? 'pending');
      ...
      >
        <View style={styles.sheet}>
          ...
          <View style={styles.header}>
            ...
            <Text style={styles.headerTitle}>협찬 상세</Text>
...
const styles = StyleSheet.create({
  ...
  statusBtn: { paddingHorizontal: 14, paddingVertical: 10, borderRadius: 10, borderWidth: 2, borderColor: 'transparent' },
